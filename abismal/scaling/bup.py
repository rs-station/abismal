...
def rms_normalize(x, axis=-1, epsilon=1e-3):
    out = x * tf.math.rsqrt(
        tf.reduce_mean(tf.square(x), axis=axis, keepdims=True) + epsilon * epsilon
    )
    return out
...
class ImageScaler(tfk.models.Model):
    ...
    def __init__(
        ...
        if optimize_prior_scale:
            self._log_k = self.add_weight('log_k', shape=(), initializer='zeros')
    ...
    def call(self, inputs, mc_samples=32, training=None, **kwargs):
        ...
        #scale_in = rms_normalize(scale_in)
        ...
        #q_latent = tf.nn.tanh(q_latent)
        #q_latent = rms_normalize(q_latent, axis=-1)
        ...
        if self.optimize_prior_scale:
            wilson_factor = tf.math.exp(-self.b * tf.math.reciprocal(tf.math.square(resolution.flat_values)) + self._log_k)
            z = z * tf.transpose(wilson_factor)
            self.add_metric(self.b, name='WilsonB')
            self.add_metric(tf.math.exp(self._log_k), name='WilsonK')

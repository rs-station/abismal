...
class ImageScaler(tfk.models.Model):
    ...
    def __init__(
        ...
        self.output_bg = tfk.layers.Dense(2, kernel_initializer=kernel_initializer, use_bias=output_bias)
        #self.built = True
    ...
    def build(self, shapes):
        ...
        self.output_bg.build(metadata[:-1] + [self.mlp_width])
        self.built = True
    ...
    def call(self, inputs, mc_samples=32, training=None, **kwargs):
        ...
        bg = self.output_bg(image)
        b = bg[...,:1]
        g = bg[...,1:]
        wilson_scale = tf.math.exp(-b * tf.math.reciprocal(tf.math.square(resolution)) + g)
        z = z * wilson_scale
        return z

...
class StreamLoaderBase(object):
    ...
    def __init__(self, filename: str, encoding="utf-8"):
        ...
        self.min_refls = 2  #drops peak lists with fewer refls
        ...
        with open(self.filename, "r") as f:
            ...
            for xmatch in self.re_crystal.finditer(data):
                ...
                for pmatch in self.re_refls.finditer(xdata):
                    ...
                    plines = pdata.split("\n")[2:-1]
                    if len(plines) < self.min_refls:
                        # Technically, we could use peak lists with a single refl, 
                        # but the downstream code needs to be modified to add a leading
                        # dimension. 
                        continue
                    ...
                    q = (A @ peak_array[:, :3].T).T #can't work with single refls
                    s1 = q + s0
...
def read_crystfel(
    ...
    **ray_kwargs,
    ...
    columns : list (optional)
        ...
        See `rs.io.crystfel.StreamLoaderBase().available_column_names` for a list of available
    ...
    loader = StreamLoaderBase(streamfile, encoding=encoding)

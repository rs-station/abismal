...
class ImageScaler(tfk.models.Model):
    ...
    def __init__(
        ...
        self.output_dense = tfk.layers.Dense(2 * 6 + 1, kernel_initializer=kernel_initializer, use_bias=output_bias)
    ...
    def call(self, inputs, mc_samples=32, training=None, **kwargs):
        ...
        hkl = tf.cast(hkl, 'float32') / 50.
        if self.hkl_to_imodel:
            image = [hkl, metadata, iobs, sigiobs]
        else:
            image = [metadata, iobs, sigiobs]
        image = tf.concat(image, axis=-1)
        ...
        params = self.output_dense(image)
        scale_bij = tfb.Exp()
        spd_bij = tfb.Chain([
        ...
        #L = tfb.FillTriangular()(params[...,:6])
        #U = tf.transpose(L, (0, 1, 3, 2))
        #U = tf.keras.ops.triu(U, 1)
        #B = U + L
        #from IPython import embed;embed(colors='linux');xx

        B = spd_bij(params[...,:6])
        S = spd_bij(params[...,6:12])
        G = scale_bij(params[...,-1])

        #B = B - tf.math.reduce_mean(B, axis=(0,1), keepdims=True)
        #G = G - tf.math.reduce_mean(G)
        #from IPython import embed;embed(colors='linux');xx

        rl = metadata.row_lengths()
        B_flat = tf.squeeze(tf.repeat(B, rl, axis=0), axis=1)
        S_flat = tf.squeeze(tf.repeat(S, rl, axis=0), axis=1)
        G_flat = tf.squeeze(tf.repeat(G, rl, axis=0), axis=1)

        dQ = metadata[...,-3:].flat_values
        hkl = hkl.flat_values 

        log_kB = -tf.squeeze(hkl[...,None,:] @ B_flat @ hkl[...,:,None], axis=(-1,-2))
        log_kP = -tf.squeeze(dQ[...,None,:] @ S_flat @ dQ[...,:,None], axis=(-1,-2))
        log_k = G_flat
        #from IPython import embed;embed(colors='linux');xx

        #log_kB = log_kB - tf.reduce_mean(log_kB)
        #log_k = log_k - tf.reduce_mean(log_k)

        log_scale = log_k + log_kB + log_kP 

        scale = tf.math.exp(log_scale) + self.epsilon
        #scale = tf.math.exp(log_k) / (1 + log_scale)
        scale = tf.RaggedTensor.from_row_splits(
            scale[...,None], metadata.row_splits
        )

        self.add_metric(tf.math.reduce_mean(scale), name='Σ_mean')
        self.add_metric(tf.math.reduce_std(scale), name='Σ_std')
        return scale
